...
cmd = [qemu, "-machine", "q35", "-accel", "tcg", "-cpu", "qemu64", "-m", "256M",
       "-smp", "1", "-drive", f"if=pflash,format=raw,readonly=on,file={firmware}",
       "-drive", f"format=raw,file={image}", "-snapshot", "-net", "none",
...
try:
    result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=60)
