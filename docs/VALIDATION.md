...
## Переименование в Generic

Обычный и smoke-образы повторно собраны после переименования; журнал:
`evidence/rename-build.txt`. Исходный `evidence/build.txt` оставлен как исторический
журнал: в нём встречается прежнее рабочее имя Iron.
